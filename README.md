...
## Phase 1i ✅ — Duplicate Audit + TNPSC Group 1 + Marks/Typewriting Clarity
- **Duplicate audit**: Ran content-aware dedup (question_ta+options for simple, assertion+reason
  for assertion_reason, table lists for match_table — not just raw text match, to avoid false
  positives on shared intro labels). Result: **0 true duplicates** across all 228 questions.
  Also ran fuzzy similarity check (>85% match) — the 5 flagged pairs were all genuinely different
  questions (vowels vs consonants, short vs long vowels, etc.), confirmed by manual inspection.
- **TNPSC Group 1**: 12 real verified questions added (`group_exam=G1`, `source_verified=1`,
  tagged `G1-2024`) — fetched from official-content source (2024 July Prelims), covering
  history, advanced Tamil literature (Thirukkural/Sangam), economy (Finance Commission),
  current affairs. Formats: assertion_reason (1), match_table (2), simple (9).
- Dashboard exam switcher now has **4 groups**: 📗 G4 · 📘 G2A · 📙 NMMS · 🏅 G1
- **New Group 1 guide** in வழிகாட்டி with full honest clarifications:
  - Prelims (200Q/300M, qualifying only, no negative marking) → Mains (750M, descriptive,
    Tamil qualifying + 3 GS papers) → Interview (100M) → Final merit out of 850
  - ⚠️ **"How many marks to win?"** — explicitly clarified there's NO fixed number; final
    cutoff varies yearly by applicants/vacancies/difficulty — directs to tnpsc.gov.in for
    reference cutoffs, refuses to fabricate a guaranteed score
  - ⚠️ **Typewriting clarity** — corrected a common misconception: typewriting/shorthand does
    **NOT** give bonus marks in TNPSC. It's a separate eligibility requirement only for
    Typist/Steno-Typist posts, unrelated to Group 1 (or Group 4/2A) exam scoring.
  - ➖ Negative marking: none in Prelims; Mains being descriptive has no MCQ-style negative
    marking either (wrong/blank = zero, not a deduction)
- Total bank: 228 questions (125 G4 + 8 G2A + 83 NMMS + 12 G1) | 15 exam guides total

## Phase 1j ✅ — 🔔 அறிவிப்புகள் (Notifications: Application Links & Vacancies)
- `migration_phase_notify.sql` — `job_notifications` table
- **Safety-critical design choice**: application links & vacancy counts can genuinely mislead
  a student if wrong, so:
  - Only `is_verified=1` rows are ever shown to students (`GET /api/notifications`)
  - **Admin panel entry is the reliable, primary path** — new "🔔 அறிவிப்பு" tab in
    `/admin/` — add/edit auto-verifies (admin is trusted)
  - A best-effort cron (`GET /api/cron/check-notifications?token=CRON_SECRET`, schedule via
    cron-job.org like the CA cron) scans TN news RSS for TNPSC-recruitment keywords and
    creates **draft** entries (`is_verified=0`) — these stay invisible to students until an
    admin reviews and hits "✅ Approve" in the panel. Auto-fetch never publishes unverified
    critical data directly.
- App: 🔔 bell icon in dashboard AppBar (red badge = count of "open" notifications) +
  dedicated Notifications tile → list screen with status badges (🟢open/🟡upcoming/⚪closed),
  vacancy count, deadline countdown (red if ≤3 days left), "Apply Now" + "Notification PDF"
  buttons (opens in external browser via `url_launcher`)
- ⚠️ Note: the seed ships with **zero notifications** — real vacancy counts/links must be
  entered via the admin panel (no fabricated placeholder data was seeded, for the same
  accuracy reasons above)

## Phase 1k ✅ — GitHub Actions: Automatic APK Build
- `.github/workflows/build-apk.yml` — push to `main` (or manual "Run workflow") automatically:
  1. Sets up JDK 17 + Flutter stable
  2. Runs `flutter create --platforms=android --org com.vetritnpsc .` — safe on an existing
     project, only adds the missing `android/` folder, never touches `lib/` or `pubspec.yaml`
  3. Patches `MainActivity.kt` → `FlutterFragmentActivity` (required for biometric login)
  4. Patches `AndroidManifest.xml` → adds notification + internet permissions
  5. `flutter pub get` → `flutter analyze` (non-blocking) → `flutter build apk --release`
  6. Uploads the APK as a 30-day build artifact **and** attaches it to an auto-created
     GitHub Release (`build-<run number>`) for a permanent download link
- ⚠️ Note: this produces a **debug-signed release APK** (Flutter's default scaffold behavior
  when no custom keystore is configured) — perfectly fine for testing/sharing with friends,
  but you'd need to add a proper release-signing config before Play Store submission
- No local Flutter installation needed on your phone/PC — just push to GitHub and download
  the APK from the Releases page a few minutes later

## Phase 1l ✅ — Single Merged SQL File
- `php_api/full_schema.sql` — all 11 migration files merged into ONE file, in verified
  correct execution order (every `ALTER TABLE` confirmed to appear after its `CREATE TABLE`)
- 9 tables, 5 ALTER statements, structurally validated
- **For a fresh Aiven database, run only this ONE file** instead of 11 separate migrations
- Individual `migration_phase*.sql` files are kept alongside for reference/incremental
  deploys if you already have a partially-set-up database
...
├── README.md
├── php_api/
│   ├── full_schema.sql               ← run this ONE file for a fresh DB
│   ├── migration_phase*.sql          ← individual migrations (reference)
│   ├── seed_loader.php, guide_seed_loader.php
│   ├── questions_seed.json, exam_guides_seed.json
│   ├── Dockerfile, .env.example
│   ├── src/Config/Database.php, Core/{Router,JWT}.php
│   ├── src/Controllers/ (9 controllers: Auth, Question, Import, Test, CA, CronCA,
│   │                      Guide, Notification, Stats, AI)
│   └── public/index.php, public/admin/index.html (admin panel)
└── flutter_app/
    ├── pubspec.yaml
    ├── assets/data/questions_seed.json, exam_guides.json
    └── lib/
        ├── main.dart, src/router.dart, src/providers/app_provider.dart
        ├── src/config/api_config.dart
        ├── src/services/ (9 services)
        ├── src/ui/widgets/ (vetri_buttons.dart, question_body.dart)
        └── src/ui/screens/ (15 screens)
```
